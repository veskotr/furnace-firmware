...
static const char* TAG = "MODBUS TRANSPORT";

modbus_master_handle_t* master_handle;

static modbus_transport_ctx_t* transport_ctx;

esp_err_t modbus_transport_init(const modbus_transport_config_t* config)
{
    ...
    CHECK_ERR_LOG_CALL_RET(mcb_master_start(master_handle),
                           modbus_transport_shutdown(),
...
esp_err_t modbus_transport_shutdown(void)
{
    if (transport_ctx == NULL || master_handle == NULL)
    {
        return ESP_OK;
    }
...

{
    ...
    return mbc_master_send_request(ctx->master_handle, &request, data);
}
