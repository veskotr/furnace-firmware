...
{
    event_manager_subscribe(DEVICE_MANAGER_EVENT, ESP_EVENT_ANY_ID, device_manager_event_handler, ctx);
    return ESP_OK;
}

esp_err_t shutdown_temp_processor_events(temp_processor_context_t* ctx)
{
    event_manager_unsubscribe(DEVICE_MANAGER_EVENT, ESP_EVENT_ANY_ID, device_manager_event_handler);
    return ESP_OK;
}
