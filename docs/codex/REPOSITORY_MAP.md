...
| `nextion_hmi` | UART protocol, UI events, program model, panel storage | RX and coordinator tasks, queues, UART/program mutexes; complete-read/atomic-draft load path; bounded packet-NAK save path | UART1 TX32/RX33, panel SD/FileStream, NVS | Many submodules/global flags; telemetry drop and storage lifecycle risks |
